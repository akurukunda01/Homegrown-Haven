-- Schema + deterministic seed for the test database (business_directory_test).
-- Built and run by backend/tests/conftest.py; never touches the real database.
...
-- Seed rows get SERIAL ids 1, 2, 3 ... in order. Coordinates are fixed (no
-- randomness) so distance-based assertions are stable.
...
-- end_date NULL => the deal is always active regardless of CURRENT_DATE.
