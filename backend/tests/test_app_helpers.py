...
import app  # importing app does not open a DB connection (get_db is lazy/per-request)
...
class TestCalculateDistance:
    ...
    def test_known_distance_within_tolerance(self):
        # Harrisburg, PA -> Lancaster, PA is roughly 35 miles.
...
class TestCleanQueryArgs:
    ...
    def test_preserves_non_numeric_keys(self):
        # q and category are never stripped here (category sentinel is handled in the route).
        args = MultiDict([("q", "coffee"), ("category", "all"), ("min_rating", "all")])
        cleaned = app._clean_query_args(args)
        assert cleaned["q"] == "coffee"
        assert cleaned["category"] == "all"
